...
public class AdminPermissionServiceImpl implements AdminPermissionService {
    ...
    public PageDataResult getPermissionList(Integer pageNum, Integer pageSize) {
        ...
        if(permissions.size() != 0){
        	 Page<PermissionDTO> page = PageHelper.startPage(pageNum, pageSize);
            PageInfo<PermissionDTO> pageInfo = new PageInfo<>(permissions);
            List<PermissionDTO> dataList = new ArrayList<PermissionDTO>();
            for(int i=0;i<permissions.size();i++) {
            	if(page.getStartRow() <= i&&page.getEndRow()>i) {
            		dataList.add(permissions.get(i));
            	}
            }
